...
class Director
{
    ...
    {
        $pdo = $this->connect();
        $stmt = $pdo->prepare("SELECT id, nombre, apellidos, fecha_nacimiento FROM directores ORDER BY id");
        $stmt->execute();
    ...
    {
        $pdo = $this->connect();
        $stmt = $pdo->prepare("SELECT id, nombre, apellidos, fecha_nacimiento FROM directores WHERE id = ?");
        $stmt->execute([$id]);

        $row = $stmt->fetch(PDO::FETCH_ASSOC);
        if (!$row) return null;

        return new Director($row["id"], $row["nombre"], $row["apellidos"], $row["fecha_nacimiento"]);
    }

    public function create($nombre, $apellidos, $fechaNacimiento)
    {
        $pdo = $this->connect();
        $stmt = $pdo->prepare("INSERT INTO directores (nombre, apellidos, fecha_nacimiento) VALUES (?, ?, ?)");
    ...
    {
        $pdo = $this->connect();
        $stmt = $pdo->prepare("UPDATE directores SET nombre = ?, apellidos = ?, fecha_nacimiento = ? WHERE id = ?");
